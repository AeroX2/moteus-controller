...
class STM32_CAN {
  ...
  public:
    ...
    STM32_CAN(FDCAN_GlobalTypeDef* canPort, CAN_PINS pins, RXQUEUE_TABLE rxSize = RX_SIZE_16, TXQUEUE_TABLE txSize = TX_SIZE_16);
    ...
    bool setFilter(uint8_t bank_num, uint32_t filter_id, uint32_t mask, IDE = AUTO);
  ...
  private:
    ...
    // void      calculateBaudrate(FDCAN_HandleTypeDef *CanHandle, int Baudrate);
    // void      setBaudRateValues(FDCAN_HandleTypeDef *CanHandle, uint16_t prescaler, uint8_t timeseg1,
    ...
    FDCAN_HandleTypeDef *n_pCanHandle;
    FDCAN_GlobalTypeDef*      _canPort;
